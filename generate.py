...
if __name__ == "__main__":
    ...
    for config in configurations:
        for n in [20, 30, 40, 50, 60, 70, 80, 150, 300, 500, 1000]:
            print(f"Generating graph with nodes {n}, p: {config[0]}, path: {config[1]}, cycle: {config[2]}, "
                  f"planar: {config[3]}, diameter: {config[4]}, locally_connected: {config[5]}, shuffle: {True}")
            global_queue.put([n, config[0], config[1], config[2], config[3], config[4], config[5], True])
